...
function sanitizeImageUrl(value) {
  ...
  try {
    ...
    const okProtocols = ['http:', 'https:', 'blob:', 'file:'];
    return okProtocols.includes(parsed.protocol) ? parsed.href : '';
  } catch (_) {
    return '';
  }
}
...
function buildFilters() {
  ...
  const addFilterButton = (label, value, isActive = false) => {
    ...
    btn.className = 'fb' + (isActive ? ' on' : '');
    btn.dataset.f = value;
    btn.textContent = label;
    btn.addEventListener('click', () => setFilter(btn, value));
    fr.appendChild(btn);
  };

  addFilterButton('Visi', 'all', true);
  addFilterButton('Vidiniai', 'indoor');
  addFilterButton('Išoriniai', 'outdoor');
  brands.forEach(b => {
    const short = b.split(' ')[0];
    addFilterButton(short, b);
  });
  const kwBtns = [2.0,2.5,3.5,5.0,7.0];
  kwBtns.forEach(k => {
    addFilterButton(`${k} kW`, `kw${k}`);
  });
}
...
function renderModels() {
  ...
  list.forEach(m => {
    ...
    const safeImageUrl = sanitizeImageUrl(m.imageUrl);
    const thumb = safeImageUrl
      ? `<img class="mc-thumb" src="${escapeHtml(safeImageUrl)}" alt="${escapeHtml(m.modelis)}" loading="lazy" />`
      : '';
    const typeLabel = modelTypeKey(m) === 'outdoor' ? 'Išorinis' : 'Vidinis';
    const energyTag = ecls && ecls !== '-' ? `<span class="tag en ${ecClass}">${escapeHtml(m.energijos_klase)}</span>` : '<span class="tag">Energija: N/A</span>';
    ...
    const widthCm = Number(m.plotis_cm) > 0 ? m.plotis_cm : 'N/A';
    const heightCm = Number(m.aukstis_cm) > 0 ? m.aukstis_cm : 'N/A';
    const depthCm = Number(m.storis_cm) > 0 ? m.storis_cm : 'N/A';

    const el = document.createElement('div');
    el.className = 'mc' + (isSel?' sel':'') + (isCmp?' cmp':'');
    el.innerHTML = `
      <div class="mc-brand">${escapeHtml(m.gamintojas)}</div>
      ${thumb ? `<div class="mc-thumb-wrap">${thumb}</div>` : ''}
      <div class="mc-name">${escapeHtml(m.modelis)}</div>
      <div class="mc-tags">
      ...
      <div class="mc-dims">📐 ${escapeHtml(widthCm)}×${escapeHtml(heightCm)}×${escapeHtml(depthCm)} cm</div>
      <div class="mc-bottom">
        <div class="mc-price">${escapeHtml(priceText)}</div>
        <div class="mc-noise">${escapeHtml(noiseText)}</div>
      </div>
      <div class="mc-sel-badge">✓</div>
      <button class="mc-cmp-btn" onclick="toggleCompare(event,${Number(m.id) || 0})">${isCmp?'✓ Lyginimui':'+ Lyginti'}</button>
    `;
...
function drawBlock(ctx, cw, ch) {
  ...
  const unitImageUrl = sanitizeImageUrl(m.imageUrl);
  if (unitImageUrl) {
    const url = unitImageUrl;
...
function openCompare() {
  ...
  cmpList.forEach(m => { html += `<th>${escapeHtml(m.gamintojas)}<br><span style="color:var(--text)">${escapeHtml(m.modelis)}</span></th>`; });
  html += `</tr></thead><tbody>`;
  ...
  fields.forEach(([label, fn, best]) => {
    ...
    cmpList.forEach((m, i) => {
      ...
      html += `<td class="${isBest?'best':''}">${escapeHtml(v)}</td>`;
    });
    html += `</tr>`;
  });
  ...
  cmpList.forEach(m => {
    html += `<button class="btn-cmp-sel" onclick="selectModel(MODELS.find(x=>x.id===${Number(m.id) || 0}));closeCompare()">Pasirinkti ${escapeHtml(m.modelis)}</button>`;
  });
  html += `</div>`;
...
function fillSummary() {
  ...
  document.getElementById('sTbl').innerHTML = rows.map(([k,v]) => `<tr><td>${escapeHtml(k)}</td><td>${escapeHtml(v)}</td></tr>`).join('');
...
function getOrderRecipientEmail() {
  const configured =
    (window.KLIMAVIZ_CONFIG && window.KLIMAVIZ_CONFIG.orderRecipientEmail) ||
    window.ORDER_RECIPIENT_EMAIL ||
    document.body?.dataset?.orderRecipientEmail ||
    '';
  return String(configured || '').trim();
}

function openEmailClient(mailtoUrl) {
  try {
...
function submitOrder() {
  ...
  const to = getOrderRecipientEmail();
  if (!to) {
    toast('Nenurodytas užsakymų el. paštas (orderRecipientEmail).', 'warn');
    return;
  }
  ...
  const emailClientOpened = openEmailClient(mailto);
  if (!emailClientOpened) {
    toast('Nepavyko atidaryti pašto kliento. Patikrinkite naršyklės nustatymus.', 'warn');
    return;
  }

  const confirmed = window.confirm('Ar pavyko atidaryti pašto programą ir paruošti laišką?');
  if (!confirmed) {
    toast('Užsakymas dar nebaigtas - forma palikta aktyvi.', 'warn');
    return;
  }

  showOrderSuccess(ref);
  toast('Užsakymas paruoštas ✓', 'ok');
}
